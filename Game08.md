<link rel="stylesheet" type="text/css" href="styles.css">

## GAME №8
Kirin vs Beginner_2017
play.abstractplay.com
Dameo. Tournament Number 2.
2024

Game URL:
[https://play.abstractplay.com/move/dameo/1/d83d3624-bd5a-4991-aada-3d25203cd14e?move=0](https://play.abstractplay.com/move/dameo/1/d83d3624-bd5a-4991-aada-3d25203cd14e?move=0)

I remember the game below for its long and intense tension of struggle.
Perhaps my partner was also under a great psychological strain, as that would explain the mistakes made by both of us.
Nevertheless, our meeting at the board turned out to be very instructive; all the mistakes will be highlighted and analysed.
Kirin got the advantage and tormented me so much that, long before the end, I was completely drained.
I was like a high-wire walker striding over an abyss, who suddenly realised that there was no safety net.
The only way was forward, but the fog made it impossible to see the nearest solid ground.

**1.e1g3       g7h6**
**2.g2h3       b8b6**
**3.d1b3       d8a5**
**4.f2h4       g8h7**

Diagr. 1
![Diagram 1](chapters/Game08/LargeDiagram-001-8.png)

As we can see, Black has decided not to fight for the h5-square and has given it to its opponent.
The defensive barrier h8-h7-h6 must compensate for this concession, whilst on the opposite flank Black aims to develop its initiative.

**5.h3h5       f6e5**

Black made this somewhat unusual move in order to prevent White from invading the g6-square.

**6.e2g4       b7b5**
**7.b3b4       b5:b3**
**8.c3:a3      d7b5**

Diagr. 2
![Diagram 2](chapters/Game08/LargeDiagram-002-16.png)

This move cannot be called a mistake, but now I would have chosen a different sequence of moves: c6b5, b5a4 and d7d5.
This would have allowed Black not only to reach the a4-square, but also to control the centre.
The move played in the game weakens the white squares in Black’s camp too much, and Kirin noticed this immediately.

**9.g1f2!      c6a4?**

Diagr. 3
![Diagram 3](chapters/Game08/LargeDiagram-003-18.png)

Black didn't pay attention to its opponent's quiet move and continued attacking on the flank.
It all makes sense, but there's no piece on d5!
And Kirin makes a sharp strike.

**10.h4g5!     h6:f4**

How many times have I used this simple, yet inconspicuous hit myself, but this time I missed it.

**11.f3:b7     b6:b8**

Diagr. 4
![Diagram 4](chapters/Game08/LargeDiagram-004-22.png)

White has won a piece, but the fight is not over yet.
Black will now do everything in its power to capture the white piece on g5 and restore material equality.
To begin with, Kirin strengthens his left flank and starts to obstruct Black's plan.

**12.b1a2      h8h6**
**13.d2d4!     ...**

Given the potential threat of f2c5, Black cannot immediately play h7h5.

**13...        b5b4!**

Diagr. 5
![Diagram 5](chapters/Game08/LargeDiagram-005-26.png)

Both opponents are demonstrating their tactical inventiveness.
Black not only parries the threat of f2c5, but also provokes White into a reckless attack with c2b3.
However, this move will be met with the counter-attack f8f6 & f7f5, capturing four of White’s checkers.

**14.f2f3!      ...**

It seems Kirin has decided to really torture me!
Once again, I can’t play h7h5 because of e3f4, which wins one more piece.
Black must urgently consolidate its rear and place a checker on the d6 square.

**14...         a8b7**
**15.a2b3!      ...**

Kirin sensed subtly that after all the exchanges, both of Black’s flanks would become vulnerable.

**15...         a4:a2**
**16.a1:a3      e7d6**
**17.b3:b5      a5:c5**

Diagr. 6
![Diagram 6](chapters/Game08/LargeDiagram-006-34.png)

Black can now recapture the lost piece, but positionally its position is considerably worse.
At this point, White could gain an overwhelming advantage with 18.d3c4 c5:c3 19.c2:c4, invading the b5, c5 or d5 squares as the situation dictated.
This line would give White a powerful attack on the left flank, and it is highly doubtful that Black would be able to save the position.
As it later turns out, Kirin had intended to play that way, but due to a technical error he made a different move, which, although weaker, also leads to a significant advantage for White.

**18.c1c3      h7h5**
**19.c2c4      h5:f5**
**20.c4:c6     d6:b6**

Diagr. 7
![Diagram 7](chapters/Game08/LargeDiagram-007-40.png)

A very instructive position has arisen.
Despite the material equality, the shortcomings of Black's position are visible to the naked eye.
This is a weak left flank, an unprotected, fragile centre, a lack of corner pieces, and a general disharmony of forces.
Defending such a position means only suffering, for even with the best moves, there is no guarantee of even a draw, and it will take a great effort, as well as a lot of nerves and emotion.
Many players would burn out here within ten moves, mentally accepting defeat.

I am a self-taught player; I have never had any checkers or chess education, coaches or mentors.
This fact leads to obvious shortcomings in my game, as I have not received any classical schooling in this sphere.
This can only be compensated for by other qualities, in particular some character traits that no one can see.
I suppose many players are in the same situation.
Usually, when I’m in a bad position, I start playing better than before.
But my God, it’s so hard to defend positions like that!

Kirin begins an offensive in the centre.
For my side, I will explain how to defend such positions.
Black’s aim is to avoid exchanges and, if the opportunity arises, to enter into a ‘clinch’, which will give some combinational possibilities.

**21.e3e4     f8f6**

First of all, the hole on square f6 should be filled.

**22.f1g2      e8e7**

Secondly, you need to group your checkers together; this will create striking columns and generally harmonise the position.

**23.h1d5?      ...**

Diagr. 8
![Diagram 8](chapters/Game08/LargeDiagram-008-45.png)

Fascinated by the plan to invade d5 with an attack in the centre, Kirin loses caution and leaves his rear completely exposed.
It was necessary not to rush, but to build up strength along line “h” by means of g3h4 & g2h3.
I think he couldn’t believe that, in this position, a combination strike could be waiting for him, crushing White’s position!
The black checkers in the centre have clenched into a fist, the power of which becomes apparent after

*23...          f5g4!!*
*24.g3:g5       e7e5*
*25.d5:f5       f6:h2*, 0:2.

However, in this tense game, the adventure is only just beginning!

**23...         e7e5?**

Believing that White is preparing to invade the e5-square, I trusted my partner and, without much thought, blocked that square.
A remarkable case of mutual blindness!
But even more striking is the fact that when I began analysing our game and reached the position in Diagram 8, I found a winning combination for Black in a few seconds!
That’s the difference between a tired mind under psychological pressure and a relaxed mind combined with a fresh perspective on the board.
But how to learn to be relaxed during the game?!
After the move 23...e7e5?, Black’s position is on the brink of defeat.
Such is the fine line between winning and losing.

Now Kirin makes a strong move, after which White’s advantage becomes overwhelming.

**24.d4d6!      ...**

Diagr. 9
![Diagram 9](chapters/Game08/LargeDiagram-009-47.png)

Black has three capture options, but none of them offers any relief.

1. *24...        e5:c5*
    *25.d6d7*, 2:0.

2.  *24...        e5:e3*
    *25.d5d7!     e6:c6*
    *26.d7e8      f5e4*
    *27.g3f4!     e3:g1*
    *28.e8:c5*, 2:0.

Try to visualise option 2 in your mind.

The third option is the most persistent and occurred in the game:

**24...         e6:c6**
**25.e4:g6      h6:f6**
**26.g2g4       ...**

Diagr. 10
![Diagram 10](chapters/Game08/LargeDiagram-010-51.png)

The Blacks' fist has fallen apart, and White is preparing a breakthrough on the right flank.
Let’s try to defend this position once more.
What can Black do?
From an analytical point of view, the situation is almost hopeless, but in every game there’s still a practical chance!
Passive defence won’t save us now; only a breakthrough plan remains, but where?
The Whites’ left flank has a fragile structure, with a gap on the b3 square.
On this flank, Black has 5 pieces against 3, so the counterattack must be launched there.

**26...         b8b5**
**27.g3f4       ...**

Diagr. 11
![Diagram 11](chapters/Game08/LargeDiagram-011-53.png)

White has tied up Black's pieces, threatening d5e6 as well as simply g4h5.
It seems that nothing can save Black now.

At that point, I spent a long time considering the obvious response 27...f7e6, but I didn’t like White’s move 28.g4h5.
In that case, White gets the king and can also strike with h5g6 in the process.
All that is true, but I missed the real gem.
But let’s go through it step by step.

*27...          f7e6*
*28.g4h5        ...*

Diagr. 12
![Diagram 12](chapters/Game08/LargeDiagram-012-55.png)

Black has four logical continuations:
attacking the d5 checker with two paths - c6c5 or e6e5;
a counterattack via e6g4 and then f6f4;
an offensive on the flank using b7b4.

A) It soon became clear that it was bad

*28...          c6c5?*
*29.a3a4        c5:e5*
*30.h5g6        f6:h6*
*31.f4:d6*, 2:0.

B) Also bad

*28...          e6e5?*
*29.d5d6!       c6:e6*
*30.h5g6        f6:h6*
*31.f4:d6*, 2:0.

C) These were straightforward options, so, given my temperament, I immediately started looking for a counter-attack

*28...          e6g4*
*29.f4:h4       f6f4*
*30.h4h6!       f4:f2*
*31.h5h7        f2f1*
*32.h6h8        ...*

Diagr. 13
![Diagram 13](chapters/Game08/LargeDiagram-013-63.png)

Here, Black can defend itself for a while, but two white kings will break through any defence.

*32...          f1h1*
*33.h8:a8       h1:h8*
*34.d5e6        ...*

Diagr. 14
![Diagram 14](chapters/Game08/LargeDiagram-014-67.png)

Despite the material equality, White is gradually winning.
White gets the second king first and eventually will torment its opponent.
I had no doubt that Kirin would win this position.

But the most amazing things are waiting for us ahead!

Diagr. 12
![Diagram 12](chapters/Game08/LargeDiagram-012-55.png)

D) As I was thinking about my move, I glanced briefly at the continuation

*28...          b7b4*
*29.a3a4        ...*

Attention!
It would be correct to play 29.h5g6! first, and only after the 2-for-2 exchange attack the piece b4 with a3a4.
Black would lose the piece b4, but its piece h6 would be moved to h4 during this time.
Black will get the king first.
I suggest readers consider this sharpest variant themselves.
Learn to analyse!

Diagr. 15
![Diagram 15](chapters/Game08/LargeDiagram-015-57.png)

I couldn’t find any good moves and left it all.
What a pity! I should have kept my eyes on the position for just another minute to find a move of tremendous power

*29...          b5b3!!*

It blows my mind!
When I discovered this in the analysis, I couldn’t sleep all night.

*30.c3:a3       e6e5*
*31.a4:c4       e5:g3*, 0:2, see for yourself!

After 29... b5b3 the other capture doesn't help either (here’s just one of the nice possibilities):

*30.a4:c4       b3:b1*
*31.h5h6        b1h1*
*32.h6g7        h1h8*
*33.d5d6        f6g5!*

Diagr. 16
![Diagram 16](chapters/Game08/LargeDiagram-016-66.png)

0:2.

However, let’s return to the actual game and Diagram 11.

Diagr. 11
![Diagram 11](chapters/Game08/LargeDiagram-011-53.png)

More amazing events await us once again!

I did not choose any of the four variations listed above.
Seeing that my opponent intends to undermine my f6 piece, I set a trap.

**27...         b7b4?!**

White now has a choice: to play cautiously with a3a4, or to get the king immediately after d5e6.
The tension had reached its peak, and I can imagine how difficult it was for Kirin to make a move at that moment.
He chose the second option, but for now let's look at the first one.
Every tempo becomes crucial, and there is no longer time to improve the position with g4h5.

*28.a3a4    ...*

Diagr. 17
![Diagram 17](chapters/Game08/LargeDiagram-017-55.png)

Now that there is no black piece on the e6 square, the combination described in point D) above does not have the desired effect:

*28...      b5b3?!*
*29.c3:a3   f7e6*
*30.a4:c4   f6e5*
*31.f4:d6   e5:g5*
*32.d6e7    g5h4*

Diagr.18
![Diagram 18](chapters/Game08/LargeDiagram-018-64.png)

White gets the king and captures the checker on h4. No traps can save Black now.
Here is a possible option:

*33.e7f8    c8b7*
*34.f8h6    h4g3*
*35.h6h2    ...*

Diagr. 19
![Diagram 19](chapters/Game08/LargeDiagram-019-69.png)

The move h2g1 threatens to capture a single piece and win the game.
Black can only set temporary traps for the white king, but nothing more.

*35...      b7a6*
*36.b2b3    b6a5*
*37.b3a4    ...*

Diagr. 20
![Diagram 20](chapters/Game08/LargeDiagram-020-73.png)

Black has run out of waiting moves; a breakthrough along the “a” line does not help – see for yourself.

Let’s continue our examination of the cautious move 28.a3a4 and return to Diagram 18:

Diagr. 17
![Diagram 17](chapters/Game08/LargeDiagram-017-55.png)

Instead of b5b3, it is possible to play b6c5:

*28...      b6c5*
*29.a4:c4   c5:e5*

Diagr. 21
![Diagram 21](chapters/Game08/LargeDiagram-021-58.png)

Perhaps Kirin didn’t like this, as White loses the d5 piece and it is impossible to undermine the f6 one.
But White has a significant positional advantage, and it is very difficult for Black to save the game.

Next after

*30.g4h5    e5e4*
*31.f4:d4   f7f4*
*32.f3e4    f6f3*
*33.e4:g4   ...*

Diagr. 22
![Diagram 22](chapters/Game08/LargeDiagram-022-65.png)

White has two extra checkers and threatens to strike with b2e5.
After 33...c6a4 34.h5h6 Black’s chances of a draw are minimal.

But let’s return to the main position in the game shown in Diagram 11:

Diagr. 11
![Diagram 11](chapters/Game08/LargeDiagram-011-53.png)

As we already know, Black played here

**27...     b7b4?!**

As we have figured out above, Kirin should have cooled his blood to zero and chosen the continuation 28.a3a4; this would have given him an excellent chance of winning.
However, the vast majority of players would, like Kirin, have made a direct strike

**28.d5e6   f6:d6**
**29.f4:f8  ...**

Diagr. 23
![Diagram 23](chapters/Game08/LargeDiagram-023-57.png)

Now, after

*29...      b6b3*
*30.f8:b8   b3:b1*
*31.d3c4    b4:d4*
*32.b8:f4   ...*

White has two extra checkers in the king's endgame and wins.

I was also going to the position shown in Diagram 23, but I had the opposite result in mind.
So what did Black come up with?

**29...     b5b3!!**

A crushing hit!
That’s how winning positions are lost and losing ones are won.

**30.f8:b5  b3:b1**
**31.b5:b2  b1:h4**

Diagr. 24
![Diagram 24](chapters/Game08/LargeDiagram-024-62.png)

**32.a3a4   h4d8**

With the help of the king and the piece c6, Black blocks a breakthrough along the ‘a’ vertical.
This seemed quite clear to White, so

0:2.

My hand refuses to mark the move 28.d5e6 with a “?”.
Yes, White fell into a non-obvious trap and lost immediately.
It was, of course, a cruel and undeserved defeat, but now Kirin and the readers are aware of this trick.
I can imagine how Kirin felt after the game and how upset he was.
However, my partner showed himself as a gentleman and congratulated me on my victory.
Just like life, any game can sometimes bring us unpleasant surprises, but the experience we gain is invaluable.
